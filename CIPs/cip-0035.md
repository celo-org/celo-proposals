...
When representing or parsing transactions in JSON format, clients SHOULD support an additional boolean field called `ethCompatible`, which indicates whether the transaction is an Ethereum-compatible transaction (`true`) or a normal Celo transaction (`false`).  This is necessary in order to know what should be used as the transaction's "signing data" (as described above).  If the field is absent in the JSON being parsed, clients SHOULD treat the transaction as if `ethCompatible` were `false`.

Note that this addition represents a change from how Celo transactions are currently represented as JSON.
