# CIP [0016]: Standard celo protocol URI scheme
...
`prefix` is used for differentiating between user and contract `target`s and actions. `pay` is for user targets and is intended for use with phone numbers or addresses and the `token` endpoint. `call` & `send` are for contract targets and intended for use with the registry/addresses and the `function` endpoint. 
