...
Starting at the first epoch after `HARDFORK_BLOCK`, block creation and validation MUST read the lookback window from chain state by calling the `BlockchainParameters` contract, rather than reading chain paramaters. Reward calculations are unaffected.
...
In order for `uptimeLookbackWindow` to remain constant during an epoch, the following rules are applied:
 ...
 2. The `getter` on `BlockchainParameters.sol` for `uptimeLookbackWindow` MUST revert if there's no value for current epoch. This error signals the blockchain client to use the `defaultValue` (pre Hardfork value).
...
As the `uptimeLookbackWindow` is now governable. Work must be done to verify the value is within safe boundaries; which are defined by the following rules:
 ...
 2. `uptimeLookbackWindow` MUST BE less or equal to `epoch size - 2`
...
In the current specification, the following migration scenarios are considered:
 ...
 1. **Hardfork is activated before Smart Contract is upgraded:** On this scenario, on the first epoch after hardfork the blockchain client will fail to obtain `uptimeLookbackWindow` from the contract; and will use `DEFAULT_VALUE`. Then, when smart contract is deployed (mid-epoch), it will continue to fail until next epoch where initialized value will be used. Thus, the value remains constant during the epoch.
 2. **Smart Contract is upgraded before Hardfork is activated:** On this scenario, on the first epoch after hardfork the blockchain client will obtain `uptimeLookbackWindow` successfully from the smart contract, and use the same value during the epoch.
 3. **setUptimeLookbackWindow() is called mid-epoch:** On this scenario, the new value will only be used after epoch change, thus value will remain constant during the epoch.
