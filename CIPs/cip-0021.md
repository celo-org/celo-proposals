...
If the return value is outside the safe window, the node MUST clamp the return value to that range such that `MIN_SAFE_LOOKBACK_WINDOW <= window <= MAX_SAFE_LOOKBACK_WINDOW`. If the result is greater than the epoch size, the node MUST discard the result and use the epoch size instead.

If the `BlockchainParameters.sol` contract errors or returns 0, the node MUST ues the default value `MIN_SAFE_LOOKBACK_WINDOW`.

```python
def clamp_lookback_window(window: int) -> int:
    # clamp to range
    window = max(window, MIN_SAFE_WINDOW)
    window = min(window, MAX_SAFE_WINDOW)
    return window
