...
- `B` is the number of consecutive blocks in which an elected Attestation Service is required to complete at least one attestation.
...
5. The address hasn't submitted `SignalOnline` event in the last `S` blocks. (see [below](#signal-online)).
