...
Despite Validator uptime on Celo being phenomenal, the maintenance of the AS nodes are [sometimes neglected](https://metabase.celo-networks-dev.org/public/dashboard/b0a27650-1d62-4645-81d7-26ff7546ff0d?date_filter=past2weeks~). Today the implicit motivation to run an AS is to contribute to the health of the network which can help procure more votes from those that are interested in the success of Celo. This proposal attempts to make running an Attestation Service mandatory for all validators and make the incentive more explicit by introducing a slashing penalty in the case of prolonged AS downtime.
...
- `AttestationSlasherMinRequested` is the minimum number of requests and AS needs to receive to be eligible for slashing. 
...
```js
// Tracks the last AttestationSlasherMinRequested times an attestation was requested from an issuer (AS node)
...
2. The issuer was selected for at least `AttestationSlasherMinRequested` attestations in the past `MinAttestationSlashablePeriod` blocks.
...
In the cast that the slashed validator drops below the `validatorLockedGoldRequirements` (currently 10K CELO) the `AttestationSlasher` will slash _and_ deaffiliate the validator, requiring the validator to increase their locked gold and re-affiliating with the Validator Group. It's therefore recommended that validators buffer their locked gold with extra CELO to avoid getting deaffiliated in the case of slashing event.
...
1. Author, test, audit, and publish `AttestationSlasher`.
...
3. Slashing can only occur if the AS was selected for at least 3 attestations in the time period.
