...
discussions-to: [link](https://github.com/celo-org/celo-proposals/issues/144)
status: Draft
type: Standards
category: Ring 1
created: 2020-12-11
...
- `AttestationSlasherPenalty` Similarly we'll start conservatively with the amount of stake slashed. At the time of writing this, only two active validators have an excess stake of less than 100 CELO (85 CELO each) while all other validators have more than this. If we set a penalty of 40 CELO, then these validators can be slashed twice before incurring any loss in epoch rewards. We will start conservative with a minimum penalty of 5 CELO and increase the amount to 40 CELO once we're confident in the slashing rules.
...
We considered introducing a mechanism for the AS to signal that it was still online in the case that it never had the opportunity to be selected for an attestation. The goal of this mechanism was to add more assurance that a validator wouldn't be incorrectly slashed if they didn't have a chance to naturally prove that they were online. Instead we're mitigating this risk in the following ways:
...
3. Slashing can only occur if the AS was selected for at least one attestation in the time period.

We opted for these methods instead of the signaling mechanism, since the signaling mechanism introduces more complexity (requires a funded AS account) and provides a potential loop-hole that downed AS instances can use to prevent getting penalized.
