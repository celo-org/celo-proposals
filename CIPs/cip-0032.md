...
- `MinGlobalAttestationCompletionRate` - The minimum completion rate across all validators for rewards to be reduced for registered Attestation Services.
...
// Track global completion rate
// currEpochIndex = getEpochNumberOfBlock(block.number) % 2
// These values get reset when epoch changes (during the first attestation request)
uint256 public epochRequestCount;
uint256 public epochIndexToCompletionCount;
uint8 private currEpochIndex;
...
Each `selectIssuers()` call will update the global attestation request count and the per issuer request count. If `getEpochNumberOfBlock(block.number) % 2` is different from the `currEpochIndex`, `currEpochIndex` is updated and the global counters are reset.
...
- `AttestationServiceRewardPercentage` - Setting `.20` would allocate 15k cUSD/year to run an Attestation Service and 60 cUSD/year to run a Validator Service.
- `MinGlobalAttestationCompletionRate` - The global completion rate rarely dips below 70%. We can set this safety net at `.5` and adjust it lower if it was ever necessary.
...
- `MinNumberOfAttestationIssuers` - 40. Any lower than this and the random selection of issuers is too easy to game.
