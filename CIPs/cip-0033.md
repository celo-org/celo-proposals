...
No changes are required for the clients, thus implementation of contractkit, the cli, oracles and the likes should remain unchanged. The same tooling currently in use for cUSD can easily be extended for cEUR.
...
The proposal involves introducing another independet unit of the Mento. Therefore, the proposed introduction of cEUR will not raise conceptually new risks. The main risks are linked to the reserve being reduced through manipulations targerting the infrastructure around Mento, in particular the cEUR oracles. A second Mento unit increases this  
  
1. Same risks as cUSD, as this is an extension of the same stability mechanism.
2. The introduction of a second stable token requires including CELO/EUR reports by oracles. This includes more attack surface as this oracles can be subject to manipulation in the centralized markets.
3. The surface of attack of the reserve increases, as we now have another CELO bucket exposed.
