...
author: Nadiem Sissouno (@sissnad)
...
- Retain stableToken (for compatibility).
...
- Deploy a new cUSD proxy (e.g. cUSDProxy) pointing to the same implementation as existing stableTokenProxy.
- Deploy a cEUR proxy (e.g. cEURProxy).
...
- Retain exchange (for compatibility).
