...
Parameters 1 through 3 can be set when `GrandaMento.sol` is initialized in the contract release. `stableTokenExchangeLimits`, however, cannot be set when the contract is initialized, and require a separate set of transactions in a separate governance proposal.
